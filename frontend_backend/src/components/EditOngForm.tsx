...
import { createOng, editOng } from "@/actions/ongActions";
...
export const EditOngForm = ({ ong }: EditOngFormProps) => {
  const { data, status } = useSession();

  const [isLoading, setIsLoading] = useState(false);
  const [imageUrl, setImageUrl] = useState(ong?.image.url || "");
  ...
  return (
    <main className="grow flex items-center justify-center py-4">
      <Form {...form}>
        ...
        >
          <h1 className="font-bold text-2xl text-center">Editar ONG</h1>
